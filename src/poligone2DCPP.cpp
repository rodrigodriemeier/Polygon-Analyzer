...
// Description: C++ program to create, analyze, and transform polygons
// Date: October 2025
// ---------------------------------------------------------------
...
struct Point{
    ...
    // Returns the distance from the point to the origin (0,0)
...
struct Polygon{
    ...
    // Computes the distance between two vertices of the polygon
    double distance_between_points(int a, int b){
        return sqrt(pow(v[a].x - v[b].x, 2) + pow(v[a].y - v[b].y, 2));
    }

    // Calculates the total perimeter of the polygon
    double perimeter(){
        double perim = 0;
        int n = v.size();
        for(int i = 0; i < n; i++){
    ...
    // Displays all vertices of the polygon
    void show_polygon(){
    ...
    // Computes the polygon's area using the Shoelace formula
    double area(){
        double area = 0;
        int n = v.size();
        for(int i = 0; i < n; i++){
    ...
    // Rotates the polygon by an angle theta (degrees or radians)
    ...
    // Scales the polygon by a factor k
...
int main(){
    ...
    while(option != 0){
        ...
        if(option == 0){
            ...
            cout << "The permiter of the polygon is: " << fixed << setprecision(2)
                 << p.perimeter() << endl;
        }
        else if(option == 2){
            cout << "Insert a point: " << endl << "(Ex. of insertion: '1')" << endl;
            cin >> orig;
            cout << "The distance from the point " << orig
                 << " and the origin is " << fixed << setprecision(2)
                 << p.v[orig-1].distance_from_origine() << endl;
        }
        else if(option == 3){
            ...
            cin >> b;
            cout << "The distance between point " << a << " and point " << b << " is: "
                 << fixed << setprecision(2) << p.distance_between_points(a-1,b-1) << endl;
        }
        else if(option == 4){
            p.show_polygon();
        }
        else if(option == 5){
            cout << "The polygon's area values " << fixed << setprecision(2)
                 << p.area() << endl;
        }
        else if(option == 6){
            cout << "Centroid : (" << fixed << setprecision(2)
                 << p.centroid().x << " , " << fixed << setprecision(2)
                 << p.centroid().y << ")" << endl;
        }
        else if(option == 7){
            ...
            cout << "Insert the angle:" << endl
                 << "Please notice: 'theta' is measured from the X axis, positive counterclockwise" << endl;
            cin >> theta;
...
// Creates a polygon by reading coordinates from the user
Polygon create_polygon(Polygon p, int n){
    p.v.resize(n);
...
void create_menu(){
    ...
    cout << "                          MENU" << endl;
    cout << "0 - Finish" << endl
         << "1 - Polygon's Perimeter" << endl
         << "2 - Distance from Origin" << endl
         << "3 - Distance Between Polygon's Points" << endl
         << "4 - Show Polygon's Points" << endl
         << "5 - Polygon's Area Value" << endl
         << "6 - Polygon's Centroid" << endl
         << "7 - Translate the Polygon" << endl
         << "8 - Rotate the polygon" << endl
         << "9 - Change Polygon's Scale" << endl
         << "10 - Create a new polygon" << endl
         << "11 - Export polygon to CSV" << endl;
    cout << "------------------------------------------------------------" << endl;
}

// Exports polygon coordinates to a CSV file
void export_to_csv(const Polygon& p, const string& filename){
